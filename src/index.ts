import { app, BrowserWindow, Tray, nativeImage, Menu, clipboard, ipcMain } from 'electron';
...
import { handleGetQRcode } from './updateSession';
// This allows TypeScript to pick up the magic constants that's auto-generated by Forge's Webpack
// plugin that tells the Electron app where to look for the Webpack-bundled app code (depending on
// whether you're running in development or production).
// 下面这些常量在 TypeScript 中是不可见的，需要自己声明
...
let mainWindow: BrowserWindow | null = null // 显示代码的主窗口
let loginWindow: BrowserWindow | null = null // 显示登录二维码的窗口
...
// 在主进程初始化日志
log.initialize();
log.info('Logger initialized in main process');
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    },
    {
      label: '复制配置文件路径', click: () => {
        clipboard.writeText(configUtils.configFilePath)
      }
    },
    {
      label: '复制日志路径', click: () => {
...
const createLive = () => {
  getDanmaku(messageList, configUtils.getCurrentRoomConfig(), mainWindow).then((retLiveClient) => {
...
function registerIPCMainHanlders() {
  ipcMain.handle('getQRcode', async (_, roomid) => {
    return await handleGetQRcode(roomid, () => {
      loginWindow.close()
      closeLive()
      createLive()
    })
  ...
  ipcMain.handle('selectRoom', async (_, roomid) => {
    configUtils.setRoomConfigListByNumber(roomid)
    loginWindow.close()
    closeLive()
    createLive()
  })
